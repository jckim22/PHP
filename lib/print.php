<?php
require_once('lib/print.php');
require_once('lib/sql.php');


function print_title($conn)
{
    $title = 'WEB';
    if (isset($_GET['id'])) {
        $filterd_id = mysqli_real_escape_string($conn, $_GET['id']);
        $sql = "SELECT * FROM topic WHERE id={$filterd_id}";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        $title = $row['title'];
    }
    echo htmlspecialchars($title);;
}
function print_list($conn)
{
    $sql = "SELECT * FROM topic";
    $result = mysqli_query($conn, $sql);
    while ($row = mysqli_fetch_array($result)) {
        echo "<li><a href=\"index.php?id=" . $row['id'] . "\"><div class=\"saw active\">" . $row['title'] . "</div></a></li>";
    }
}
function print_description($conn)
{

    // if (isset($_GET['id'])) {
    //     $base = basename($_GET['id']);
    //     // echo htmlspecialchars(file_get_contents("data/" . $_GET['id']));
    //     echo strip_tags(file_get_contents("data/" . $base), '<a>,<img>,<iframe>');
    // } else {
    //     echo strip_tags(file_get_contents("data/WEB"), '<a>,<img>,<iframe>');
    // }

    $article = array(
        'title' => 'Welcome', 'description' => '<p>The World Wide Web (WWW), commonly known as the Web, is an information system enabling documents and
        other web
        resources to be accessed over the Internet.</p><br><img src="data:image/jpeg;base64,"
        alt="" width="20%">', 'created' => 0,'author'
    );

    if (isset($_GET['id'])) {
        $filterd_id = mysqli_real_escape_string($conn, $_GET['id']);
        $sql = "SELECT * FROM topic LEFT JOIN author 
        ON topic.author_id = author.id WHERE topic.id={$filterd_id}";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        $base = basename($filterd_id);
        // echo htmlspecialchars(file_get_contents("data/" . $_GET['id']));
        $article['title'] = $row['title'];
        $article['created'] = $row['created'];
        $article['description'] = $row['description'];
        $article['author'] = $row['name'];
        echo strip_tags("<h3>게시 날짜:" . $article['created'] . "</h3><br><br><h4>By: {$article['author']}</h4>", '<h4><h3>,<br>,<a>,<img>,<iframe>');

    }


    echo "<h2>" . strip_tags($article['title'] . "</h2>" . " <br>", '<h2>,<br>,<a>,<img>,<iframe>');

    echo strip_tags("게시 글:" . $article['description'] . "<br>", '<br>,<a>,<img>,<iframe>');
}
function print_description_update($conn)
{

    // if (isset($_GET['id'])) {
    //     $base = basename($_GET['id']);
    //     // echo htmlspecialchars(file_get_contents("data/" . $_GET['id']));
    //     echo strip_tags(file_get_contents("data/" . $base), '<a>,<img>,<iframe>');
    // } else {
    //     echo strip_tags(file_get_contents("data/WEB"), '<a>,<img>,<iframe>');
    // }


    if (isset($_GET['id'])) {
        $filterd_id = mysqli_real_escape_string($conn, $_GET['id']);
        $sql = "SELECT * FROM topic WHERE id={$filterd_id}";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        $base = basename($filterd_id);
        $article = $row['description'];
        echo strip_tags($article, '<br>,<a>,<img>,<iframe>');
    }
}
function print_description_author_update($conn)
{

    // if (isset($_GET['id'])) {
    //     $base = basename($_GET['id']);
    //     // echo htmlspecialchars(file_get_contents("data/" . $_GET['id']));
    //     echo strip_tags(file_get_contents("data/" . $base), '<a>,<img>,<iframe>');
    // } else {
    //     echo strip_tags(file_get_contents("data/WEB"), '<a>,<img>,<iframe>');
    // }


    if (isset($_GET['id'])) {
        $filterd_id = mysqli_real_escape_string($conn, $_GET['id']);
        $sql = "SELECT * FROM author WHERE id={$filterd_id}";
        $result = mysqli_query($conn, $sql);
        $row = mysqli_fetch_array($result);
        $base = basename($filterd_id);
        $article =array('name'=>$row['name'],'profile'=>$row['profile']);
        echo strip_tags($article['name'], '<br>,<a>,<img>,<iframe>');
        echo strip_tags($article['profile'], '<br>,<a>,<img>,<iframe>');
    }
}
